...
if total_score > 0:
    ...
    print(f"Reanimation % of total: {reanimation_score} pts → {percent:.2f}%")
    ...
    if percent < target_percentage:
        ...
        print(f"⚠ To reach {target_percentage}% reanimation score, you need at least {required_points} more reanimation points.")
    else:
        print(f"✅ Reanimation goal of {target_percentage}% achieved!")

    # Part 1 – Target total score
    if total_score < target_total_score:
        more_needed = target_total_score - total_score
        print(f"📈 You need {more_needed} more points to reach the target total score of {target_total_score}.")
    else:
        print(f"✅ Total score target of {target_total_score} reached.")
    ...
    if reanimation_score / total_score >= required_ratio:
        print(f"✅ Current reanimation score already meets {int(required_ratio * 100)}% threshold.")
    elif min_needed_removal <= max_removable:
        print(f"🗑️ You can delete at least {min_needed_removal} non-reanimation points ({min_needed_removal} worth of screenshots)")
        print(f"   to meet the {int(required_ratio * 100)}% reanimation threshold.")
    else:
        print(f"❌ You cannot reach {int(required_ratio * 100)}% reanimation threshold by deleting screenshots alone.")



else:
    print("No files found. Total score is 0.")
